...
function NewPatient({ onCancel }: { onCancel: () => void }) {
  return <div className="new-patient-page"><div className="section-heading"><div><p className="eyebrow">Медицинский архив</p><h1>Новая медицинская история</h1><p className="muted">Добавьте основные сведения о пациенте, чтобы начать оценку состояния</p></div><span className="draft-badge"><span className="saved-dot" /> Черновик</span></div><div className="new-patient-layout"><section className="card form-card new-patient-form"><div className="card-title-row"><div><h2>Основные сведения</h2><p className="muted">Поля, отмеченные *, обязательны для заполнения</p></div></div><div className="fields-grid"><label className="input-field"><span>Фамилия *</span><input placeholder="Например, Морозов" autoFocus /></label><label className="input-field"><span>Имя *</span><input placeholder="Например, Алексей" /></label><label className="input-field"><span>Отчество</span><input placeholder="Например, Петрович" /></label><label className="input-field"><span>Идентификатор истории *</span><input placeholder="Например, 10415/2024" /></label><label className="input-field"><span>Дата рождения *</span><input type="date" /></label><label className="input-field"><span>Пол *</span><select defaultValue=""><option value="" disabled>Выберите пол</option><option>Мужской</option><option>Женский</option></select></label></div></section><aside className="card new-patient-guide"><div className="support-icon"><FileHeart size={18} /></div><h2>Следующий шаг</h2><p>После сохранения истории вы сможете внести показатели анализов и рассчитать индекс тяжести.</p><div className="guide-step"><b>1</b><span>Заполните данные пациента</span></div><div className="guide-step"><b>2</b><span>Добавьте клинические показатели</span></div><div className="guide-step"><b>3</b><span>Подтвердите экспертную оценку</span></div></aside></div><div className="action-row"><button className="button secondary" onClick={onCancel}>Отмена</button><button className="button primary" onClick={onCancel}><Plus size={16} /> Сохранить историю</button></div></div>;
}

function Archive({ onNewPatient }: { onNewPatient: () => void }) {
  return <div className="archive-page"><div className="section-heading"><div><p className="eyebrow">Медицинский архив</p><h1>Пациенты</h1><p className="muted">24 активные медицинские истории</p></div><button className="button primary" onClick={onNewPatient}><Plus size={17} /> Новая история</button></div><div className="toolbar"><div className="search-box"><Search size={17} /><input placeholder="Поиск по ФИО или идентификатору" /></div><button className="filter-button"><Filter size={16} /> Фильтры <span className="filter-count">2</span></button><button className="filter-button"><SlidersHorizontal size={16} /> Настроить вид</button></div><div className="card table-card"><div className="table-wrap"><table><thead><tr><th>Пациент</th><th>Идентификатор</th><th>Дата анализа</th><th>Тяжесть</th><th>Врач</th><th>Статус</th><th /></tr></thead><tbody>{patients.map((patient) => <tr key={patient.id}><td><div className="table-patient"><span>{patient.name.split(" ").map((word) => word[0]).join("")}</span><strong>{patient.name}</strong></div></td><td className="mono">{patient.id}</td><td>{patient.date}</td><td><b className={patient.status === "Высокая" ? "score high-score" : "score"}>{patient.score}</b></td><td>Е. Мангальова</td><td><span className={patient.status === "Высокая" ? "status-pill status-amber" : "status-pill status-green"}><span className="status-dot" />{patient.status}</span></td><td><MoreHorizontal size={18} /></td></tr>)}</tbody></table></div><div className="table-footer"><span>Показано 3 из 24</span><button className="text-button">Загрузить ещё</button></div></div></div>;
}
